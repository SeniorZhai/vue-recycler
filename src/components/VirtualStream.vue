  ...
  methods: {
    ...
    getPreviousItemHeight(item) {
      ...
      const previousId = (() => {
        const id =
          previousIndex !== false
            ? this.identifier.indexes[previousIndex]
            : false;
        return id ? id : false;
      })();
    ...
    updateItemDimensions() {
      ...
      this.totalHeight = !dimensionArray.length
        ...
        : dimensionArray.reduce((dimensionA, dimensionB) => {
            const aVal =
              typeof dimensionA === "object" ? dimensionA.height : dimensionA;
            const bVal =
              typeof dimensionB === "object" ? dimensionB.height : dimensionB;
            return aVal + bVal;
          });
          ...
          if (this.oldScrollTop !== 0) {
            if (oldTotalHeight < this.totalHeight) {
              ...
              window.requestAnimationFrame(() => {
                ...
                this.$refs.wrapper.style["-webkit-overflow-scrolling"] =
                  "touch";
              });
            }
          }
    ...
    updateAllDimensions() {
      ...
      Object.keys(this.dimensions).forEach((key, i) => {
        ...
        Object.assign(this.dimensions[key], {
    ...
    getCurrentView(position, items) {
      ...
      const endPos =
        position + this.count > items.length
          ? items.length
          : position + this.count;
  ...
  watch: {
    ...
    itemCount(newVal, oldVal) {
      if (newVal !== oldVal) {
        ...
        const index = !this.reverseItems
          ? 0
          : this.renderedItems.length - diffCount;
      ...
      this.$nextTick(() => {
        if (newVal > oldVal) {
          if (this.attachToStart && this.scrollAttachedTo === "start") {
            this.$refs.wrapper.scrollTop = !this.reversed
              ? 0
              : this.totalHeight;
          }
      ...
      window.addEventListener(
        ...
        debounce(() => {
          ...
          if (windowSize.width !== this.oldWindowSize.width) {
            ...
            this.updateItemDimensions();
            this.$refs.wrapper.style["-webkit-overflow-scrolling"] = "auto";
            if (this.$refs.wrapper && this.reversed)
